...
layui.define(['jquery', 'layer'], function (exports) {
  ...
  var obj = function obj(config) {
    this.disabledIndex = []; //当前选中的值名数据

    this.selected = []; //当前选中的值
    ...
    this.config = $.extend(this.config, config); //创建选项元素
    ...
    this.createOption = function () {
      ...
      html += '<input ' + verify + ' type="text" name="" readonly="" class="layui-input layui-unselect">';
      html += '</div>';
    ...
    this.set = function (selected) {
      ...
      $E.find('dd').removeClass('layui-this'); //为默认选中值添加类名
    ...
    this.setSelected = function (first) {
      ...
      if (items.length == 0) {
        ...
        items.each(function () {
          ...
          spans.push('<input type="hidden" ' + c.verify + ' name="' + c.name + '" readonly="" value="' + v + '" class="layui-input layui-unselect">');
          selected.push(item);
        });
      }
  ...
  obj.prototype.render = function () {
    ...
    $E.on('click', '.layui-select-title,.multiple,.multiple.layui-edge', function (e) {
      ...
      if ($(c.elem).is('.layui-form-selected')) {
        ...
        $(document).on('click', mEvent = function (_mEvent) {
          function mEvent(_x) {
            return _mEvent.apply(this, arguments);
          }

          mEvent.toString = function () {
            return _mEvent.toString();
          };
